        ...
        with torch.no_grad():
            with tqdm(range(num)) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    out_gan = self.vqgan(imgs)
                    gan_activs = self.store_activs(self.vqgan, self.vqgan_layers)
                    vqgan_feature = gan_activs[0] #[1,256,16,16]
                    print('vqgan_feature.shape:',vqgan_feature.shape)
                    # #根据匹配分数选择与vqgan feature匹配的神经元
                    ganlayers, discrlayers = layers.get_layers(self.vqgan, self.vqgan_layers, self.source_model, self.source_layers, imgs, args)
                    # print(ganlayers) #'quant_conv':256
                    ...
                    print('len(gan_match_units):',len(gan_match_units))
                    for unit in range(table.shape[0]):
                        if args.model_mode=='clip':
                            out_sr = self.source_model.model.encode_image(imgs)
                        else:
                            out_sr = self.source_model(imgs)
                        ...
                        # print('unit_match_score:',unit_match_score)
                        # if unit_match_score>0.7:
                    ...
                    # x_q, emb_loss, _ = self.vqgan_net.endecoder.top.quantize(vqgan_feature)
                    x_q, emb_loss, _ = self.vqgan_net.endecoder.top.quantize(matching_features)
                    x = self.vqgan_net.endecoder.top.decode_convin(x_q)
                    x = self.vqgan_net.endecoder.top.decode_atten_1(x)
                    x = self.vqgan_net.endecoder.top.decode_atten_2(x)
                    x = self.vqgan_net.endecoder.top.decode_0(x)
                    x = self.vqgan_net.endecoder.top.decode_1(x)
                    x = self.vqgan_net.endecoder.top.decode_2(x)
                    x = self.vqgan_net.endecoder.top.decode_3(x)
                    x = self.vqgan_net.endecoder.top.decode_4(x)

                    x = self.vqgan_net.endecoder.norm_out(x)
                    def nonlinearity(x):
                        # swish
                        return x*torch.sigmoid(x)
                    x = nonlinearity(x)
                    decoded_images = self.vqgan_net.endecoder.conv_out(x)
                    
                    # decoded_images, _ =  self.vqgan_net(imgs)

                    if i % 1 == 0:
                        with torch.no_grad():
                            # real_fake_images = torch.cat((imgs[:1], decoded_images.add(1).mul(0.5)[:1]))
                            real_fake_images = torch.cat((imgs[:1], decoded_images[:1]))
                            vutils.save_image(real_fake_images, os.path.join(args.save_path, f"{i}_recon_match.jpg"), nrow=4)
...
if __name__ == '__main__':
    ...
    args.model_mode = "resnet50"
    args.save_path = "/home/xjiangbh/ModelZoo_work/VQD-SR/taming-transformers/results_compare/matching_resnet50_alllayers/"
